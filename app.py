...
def retrieve_total_travel_time(number_stop: int, driving_time: int, charging_time: int):
    ...
    time = client.service.travel_time(
        int(number_stop), int(driving_time), int(charging_time)
    )
    return time
...
def main():
    ...
    if origin and destination and car_id:
        ...
        direction = MapboxDirection(
            origin_place.get_position(), destination_place.get_position()
        )
        ...
        new_direction = MapboxDirection(
            origin_place.get_position(),
            destination_place.get_position(),
            list_positions,
        )

        charging_time_car = 30 * 60
        charging_time = strfdelta(
            datetime.timedelta(seconds=charging_time_car * len(list_positions))
        )
        total_time = strfdelta(
            datetime.timedelta(
                seconds=retrieve_total_travel_time(
                    len(list_positions), new_direction.get_duration(), charging_time_car
                )
            )
        )
    ...
    return render_template(
        ...
        ACCESS_KEY=Config.MAPBOX_ACCESS_KEY,
        origin=origin,
        destination=destination,
        waypoints=list_positions,
        waypoints_markers=waypoints_markers,
